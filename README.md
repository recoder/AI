...
The initial bootstrap foundation creates workspace directories and reconciles helper packages. Music tool installers and runners are planned.

With PowerShell 7.4+ and this repository available locally, run `./bootstrap.ps1`. For an existing `just` setup, run `just setup`, `just plan`, and `just bootstrap`, then `just doctor`. See the [bootstrap guide](docs/bootstrap.md) for prerequisite installation, configuration, and recovery.
